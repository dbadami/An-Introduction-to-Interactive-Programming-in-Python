...
"""8 rocks instead of 12 rocks have been spawned because spawning
12 rocks causes the game run a bit slow in the later stages. The number
of rocks to be spawned can be changed below"""
rocks_to_spawn = 8
...
#Process_Sprite_Group Helper Function
...
#Group Collide Helper Function (Rocks and Ship)
    ...
    for item1 in group1:
        for item2 in group2:
            if item1.collide(item2):
                score+=10
                remove_set1.add(item1)
        group_collide(group2,item1)
...
def draw(canvas):
    ...
    if started:
        #Drawing All of the Asteroids and Missiles
        process_sprite_group(rock_group, canvas)
    process_sprite_group(missile_group, canvas)
    process_sprite_group(explosion_group,canvas)
        
    #Checking Collisions between the rock and the ship
...
def rock_spawner():
    if len(rock_group)<rocks_to_spawn:
        rock_pos = [random.randrange(0, width), random.randrange(0, height)]
        rock_vel = [(random.random() * .6 - .3)+(random.randrange(-1,2,2))*(score//50)*0.5, (random.random() * .6 - .3)+(random.randrange(-1,2,2))*(score//50)*0.5]
        rock_avel = (random.random() * .2 - .1)
        if dist(rock_pos, my_ship.pos)<=asteroid_info.get_radius()+my_ship.get_radius():
            rock_pos[0]+=dist(rock_pos, my_ship.pos)*5
            rock_pos[1]+=dist(rock_pos, my_ship.pos)*5
        a_rock = Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info)
        rock_group.add(a_rock)  
        
# Helper function that runs when all the lives have been lost
def reset():
